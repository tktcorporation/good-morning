...
- dayBoundaryHour がアラーム時刻より後の設定で、境界通過前に「明日だけ変更」画面を開くとピッカーの表示（今日の曜日設定）と実際の保存先（1日先送りされた override 対象日）がズレる問題を修正
- 起床履歴・セッションストアの読み込みが失敗すると loaded フラグが永久に立たず、以降アラーム無効化・時刻変更・「明日だけ変更」保存などの明示的な操作をしてもネイティブアラーム同期が二度と走らなくなる問題を修正
