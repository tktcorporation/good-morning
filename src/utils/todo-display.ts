 ...
 * 表示用の TODO タイトルを取得する。
 *
 * 背景: 固定種別タスク（'squat'/'sky' 等）の title は永続化時点のロケールに依存しない
 * 英語リテラル（FIXED_SQUAT_TODO_TITLE / FIXED_SKY_TODO_TITLE 参照）を保存しているため、
 * UI でそのまま render するとロケール変更後にロケール混在が発生する。
 * type 既知のものは i18n でロケライズし、未設定（レガシーデータ）は title を素通し。
 *
 * 呼び出し箇所:
 * - SquatChallengeItem / SkyChallengeItem / day-review (アプリ内 UI)
...
export function getLocalizedTodoTitle(todo: {
  ...
  if (todo.type === 'sky') {
    return i18n.t('morningRoutine.sky.title', { ns: 'dashboard' });
  }
  return todo.title;
}
